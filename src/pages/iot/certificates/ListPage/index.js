...
class CertificatePage extends React.Component {
  ...
  render() {
    const columns = [
      {
        ...
        width: '30%',
        render: (text, record) => (
          <a className="link" href={`#/certificates/${record.id}`}>
            {record.name}
          </a>
        ),
      },
    ...
    const content = (
      <div>
        <Popconfirm
          title="Are you sure delete these certificates? You cannot rollback."
        ...
        <section className="card">
          <div className="card-header">
            <div className="utils__title">
              <strong>Certificates Management</strong>
            </div>
            <small>
          ...
          <div className="card-body">
            ...
            {(!totalItems || totalItems <= 0) && (
              <LockScreenPage name=" Certificate" link="#/certificates/create" />
            )}
          </div>
        </section>
      </div>
